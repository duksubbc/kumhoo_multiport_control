...
u8 ir_update_ids(u8 idx)
{
  ...
  FLASH_Unlock(FLASH_MEMTYPE_DATA);
  FLASH_ProgramByte((u32)&_eeprom_ids[j],(u8)low);
  FLASH_ProgramByte((u32)&_eeprom_ids[j+1],(u8)hi);
  FLASH_ProgramByte((u32)_eeprom_idx,(u8)idx);
  //_eeprom_ids[j] = low;
  //_eeprom_ids[j+1] = hi;
  //_eeprom_idx = idx;
  FLASH_Lock(FLASH_MEMTYPE_DATA);
...
void Init_IR(void)
{
  ...
  GPIO_Init(IR_PORT, (GPIO_Pin_TypeDef)(IR_Rx), GPIO_MODE_IN_FL_IT);
