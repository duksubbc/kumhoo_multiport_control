...
INTERRUPT_HANDLER(UART2_RX_IRQHandler, 21)
{
	...
	if((rxBuf[0] == MAINSUB_STX)
		&& ((rxBuf[1] == deviceID) || (rxBuf[1] == MULTI_ID)))
	{
		// checksum calc.
		checksum = MAINSUB_STX;
		for(i=1; i<(SUB_PROTOCOL_LEN-1); i++)
			checksum += rxBuf[i];

		if(checksum != rxBuf[SUB_PROTOCOL_LEN-1])
		{
			return;
		}
		...
		for(i=0; i<(SUB_PROTOCOL_LEN-1); i++)
			rxBuf[i] = 0;
		return;
	}
#endif
