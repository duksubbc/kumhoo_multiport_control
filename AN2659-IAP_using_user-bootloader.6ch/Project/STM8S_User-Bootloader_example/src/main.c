...
void main(void){
  ...
  GPIOD->DDR |= 0x40; 	  // PD2 as Input 
  GPIOD->ODR &= ~0x40; 	  // PD2 as Input 
  GPIOD->DDR &= ~0x40; 	  // PD2 as Input 
  //GPIOD->CR1 |= 0x20; 		// PD2 as Pull Up
  GPIOD->CR2 &= ~0x40;	  // no external interrupt
	
  // Detect if bootloader is enabled (pin in high state)
  if( (GPIOD->IDR & 0x40) != 0x40 ) 
  {
...
u8 WM_Command_Process(void){
  ...
  //DataAddressDebug[DataAddressidx++] = DataAddress;
  
  //DataAddress = (u8 FAR*)0xA0;
